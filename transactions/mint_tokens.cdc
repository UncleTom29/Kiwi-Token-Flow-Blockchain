...
transaction(recipient: Address, amount: UFix64) {
    ...
    /// Reference to the KiwiToken Admin Resource object
    let tokenAdmin: &KiwiToken.Administrator
    ...
    prepare(signer: AuthAccount) {
        ...
        self.tokenAdmin = signer.borrow<&KiwiToken.Administrator>(from: KiwiToken.AdminStoragePath)
            ?? panic("Signer is not the token admin")

        // Get the account of the recipient and borrow a reference to their receiver
        self.tokenReceiver = getAccount(recipient)
            .getCapability(KiwiToken.ReceiverPublicPath)
    ...
    post {
        KiwiToken.totalSupply == self.supplyBefore + amount: "The total supply must be increased by the amount"
    }
}
